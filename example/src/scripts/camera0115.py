...
class CameraHandler():
    ...
    def __init__(self):
        ...
        self.class_names = ['oneway', 'highwayexit', 'stopsign', 'roundabout', 'park', 'crosswalk', 'noentry', 'highwayentrance', 'priority',
                ...
                'others','others','others','others','others','others','others','others','others','others',
                'others','others','others','others','others','others','others','others','others','others',]
        self.stop_sizes = [50,50,55,50,50,65,50,50,60,25,50,70,100]
        self.detected_id = 0
    ...
    def callback(self, data):
        ...
        # print("image size: ", img_lines.shape)
        self.detect_object()
        ...
        #     if state.data==0:
        #         cv2.putText(self.imgL, self.class_names[self.detected_id]+' detected!'+'(red)', (int(self.imgL.shape[1]*0.1),int(self.imgL.shape[0]*0.1)), cv2.FONT_HERSHEY_SIMPLEX, 
        #            1, (0,0,255), 1, cv2.LINE_AA)
        #         self.detected_id=0
        #     elif state.data==1:
        #         cv2.putText(self.imgL, self.class_names[self.detected_id]+' detected!'+'(yellow)', (int(self.imgL.shape[1]*0.1),int(self.imgL.shape[0]*0.1)), cv2.FONT_HERSHEY_SIMPLEX, 
        #            1, (0,255,255), 1, cv2.LINE_AA)
        #         self.detected_id=0
        #     else:
        #         cv2.putText(self.imgL, self.class_names[self.detected_id]+' detected!'+'(green)', (int(self.imgL.shape[1]*0.1),int(self.imgL.shape[0]*0.1)), cv2.FONT_HERSHEY_SIMPLEX, 
        #            1, (0,255,0), 1, cv2.LINE_AA)
        #         self.detected_id=0
        # self.imgL = self.yolov7_detector.draw_detections(self.imgL)
    ...
    def detect_object(self):
        # object detector
        self.boxes, self.scores, self.class_ids = self.yolov7_detector(self.imgL.copy())
        self.imgL = self.yolov7_detector.draw_detections(self.imgL)
        # check detected objects
        if any(self.class_ids): #not empty
            id = self.class_ids[0]
            cv2.putText(self.imgL, self.class_names[self.detected_id]+' detected!', (int(self.imgL.shape[1]*0.1),int(self.imgL.shape[0]*0.1)), cv2.FONT_HERSHEY_SIMPLEX, 
                   1, (0,0,255), 1, cv2.LINE_AA)
            if id<=12:
