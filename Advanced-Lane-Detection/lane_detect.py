...
def warp(img, show = False):
    ...
    offset = 300 
    original_size = np.array([1280, 720])
    scale_factor = img_size / original_size
    print("scale_factor: ", scale_factor, "img_size: ", img_size, "original_size: ", original_size)
    # Source points taken from images with straight lane lines, these are to become parallel after the warp transform
    src = np.float32([ #test2
        (20, 720), # bottom-left corner
        (471, 270), # top-left corner
        (810, 270), # top-right corner
        (1245, 720) # bottom-right corner
    ])
    # src = np.float32([ #original
    #     (190, 720), # bottom-left corner
    #     (596, 447), # top-left corner
    #     (685, 447), # top-right corner
    #     (1125, 720) # bottom-right corner
    # ])
    src = np.float32(src * scale_factor)
    offset = int(offset * scale_factor[0])
    ...
    if show:
        for point in src:
            cv2.circle(undist, tuple(point.astype(int)), 5, (0, 0, 255), -1)
        cv2.polylines(undist, [np.int32(src)], True, (255, 0, 0), 2)
        for point in dst:
            cv2.circle(warped, tuple(point.astype(int)), 5, (0, 255, 0), -1)
        cv2.polylines(warped, [np.int32(dst)], True, (0, 255, 0), 2)

        # Display the original and warped images
        cv2.imshow('Original', undist)
        cv2.imshow('Warped', warped)

        cv2.waitKey(0)
        cv2.destroyAllWindows()

    return warped, M_inv, src
...
img = cv2.imread('test2/test9b.JPG')
img = cv2.imread('test2/b1.png')
print("shape: ", img.shape)

def binary_thresholded(img, yellow=False):
...
# Camera parameters from the XML
# h_fov = 1.085595
# width = 640
# height = 480
...
# # Assume the optical center is at the center of the image
# c_x = width / 2
# c_y = height / 2
...
# # The distortion coefficients are all zero
# dist = np.zeros((5,), dtype=np.float32)


binary_thresh = binary_thresholded(img)
out_img = np.dstack((binary_thresh, binary_thresh, binary_thresh))*255
binary_warped, M_inv, src = warp(binary_thresh)
for point in src:
    cv2.circle(out_img, tuple(point.astype(int)), 5, (0, 0, 255), -1)
cv2.polylines(out_img, [np.int32(src)], True, (255, 0, 0), 2)
plt.imshow(out_img, cmap='gray')
plt.show()
# cv2.imshow('binary', out_img)
# cv2.waitKey(0)
...
left_fit, right_fit, left_fitx, right_fitx, ploty = fit_poly(binary_warped,leftx, lefty, rightx, righty)
out_img = draw_poly_lines(binary_warped, left_fitx, right_fitx, ploty)
plt.title('Previous Polynomial Search')
plt.imshow(out_img)
plt.show()
